...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .shell{display:flex;gap:26px;align-items:flex-start;margin-top:18px}
  .side{width:198px;flex:none;position:sticky;top:18px}
  .sidelab{font-size:11px;letter-spacing:.06em;text-transform:uppercase;color:var(--faint);padding:0 12px 8px}
  .navitem{display:flex;align-items:center;justify-content:space-between;gap:8px;width:100%;text-align:left;background:none;border:none;cursor:pointer;font-family:var(--sans);font-size:14px;color:var(--dim);padding:9px 12px;border-radius:8px;border-left:2px solid transparent;margin-bottom:2px}
  .navitem:hover{color:var(--ink);background:var(--inset)}
  .navitem.active{color:var(--ink);background:var(--accent-soft);border-left-color:var(--accent)}
  .navitem .nm{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .navitem .cnt{font-family:var(--mono);font-size:11px;color:var(--faint);background:var(--inset);padding:1px 7px;border-radius:99px;flex:none}
  .navitem.active .cnt{color:var(--accent);background:var(--panel)}
  .main{flex:1;min-width:0}
  @media (max-width:720px){
    .shell{flex-direction:column;gap:14px;margin-top:14px}
    .side{width:100%;position:static}
    .sidelab{display:none}
    nav{display:flex;gap:6px;overflow-x:auto;padding-bottom:4px;border-bottom:1px solid var(--line)}
    .navitem{width:auto;border-left:none;border-bottom:2px solid transparent;border-radius:8px 8px 0 0;flex:none;margin-bottom:-1px}
    .navitem.active{border-left:none;background:none;border-bottom-color:var(--accent)}
    .navitem.active .cnt{background:var(--accent-soft)}}

  .legend{display:flex;align-items:center;gap:16px;font-size:12px;color:var(--dim);margin-bottom:6px;flex-wrap:wrap}
  .legend i{display:inline-block;width:11px;height:11px;border-radius:3px;margin-right:6px;transform:translateY(1px)}
  .hint{font-size:12px;color:var(--faint);margin-bottom:16px}

  .gantt{display:flex;background:var(--panel);border:1px solid var(--line-2);border-radius:var(--radius);padding:12px 14px 14px;margin-bottom:18px;overflow:hidden}
  .gutter{width:172px;flex:none}
  .ghdr{height:22px}
  .gcell{height:30px;display:flex;align-items:center;font-size:12px;color:var(--dim);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding-right:10px;cursor:pointer}
  .gcell:hover{color:var(--ink)}
  .gcell.on{color:var(--accent)}
  .gcell .gid{font-family:var(--mono);color:var(--faint);margin-right:6px}
  .gcell.on .gid{color:var(--accent)}
  .plot{flex:1;position:relative;min-width:0}
  .axis{position:relative;height:22px}
  .axis .tick{position:absolute;transform:translateX(-50%);font-family:var(--mono);font-size:10.5px;color:var(--faint);white-space:nowrap;top:2px}
  .bands{position:relative}
  .grid{position:absolute;top:0;bottom:0;width:1px;background:var(--line-2)}
  .gbar{position:absolute;height:16px;min-width:6px;border-radius:4px;cursor:pointer;overflow:hidden;border:1px solid transparent}
  .gbar:hover{border-color:var(--accent)}
  .gbar.on{border-color:var(--accent);box-shadow:0 0 0 2px var(--accent-soft)}
  .segbar{display:flex;width:100%;height:100%;background:var(--inset)}
  .sg{display:block;height:100%}.sg1{background:var(--ph1)}.sg2{background:var(--ph2)}.sg3{background:var(--ph3)}
  .sg-open{flex:1;min-width:10px;background:repeating-linear-gradient(45deg,var(--run-soft),var(--run-soft) 4px,transparent 4px,transparent 8px);border-right:2px solid var(--run)}

  .selbox{background:var(--panel);border:1px solid var(--accent);border-radius:var(--radius);padding:16px 18px}
  .selbox.empty{border-color:var(--line-2);color:var(--faint);font-size:13px;text-align:center;padding:24px}
  .selhead{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:6px}
  .selid{font-family:var(--mono);font-size:13px;color:var(--dim)}.selid b{color:var(--ink)}
  ...
  .seltitle{font-size:16px;font-weight:600;margin-bottom:4px}
  .selspan{font-family:var(--mono);font-size:12.5px;color:var(--dim);margin-bottom:14px}.selspan b{color:var(--accent);font-weight:600}
...
<div class="wrap">
  ...
  <div class="hint">一列一個 issue,依開始時間排序。bar 橫跨的就是它佔掉的那幾天;不同列的 bar 落在同一段水平區間 = 那幾天平行在弄。點 bar 或左側名稱看細節。</div>
  <div id="chart"></div>
  <div id="sel"></div>
    </main>
  </div>
...
function renderNav(){
  ...
  document.querySelectorAll('.navitem').forEach(b=>b.onclick=()=>{cur=+b.dataset.i;sel=null;render();});
}

function render(){
  renderNav();
  const p=PROJECTS[cur];
  const rows=[...p.issues].map(it=>({it,d:computeDurations(it)})).sort((a,b)=>a.d.tNew-b.d.tNew);
  const min=Math.min(...rows.map(r=>+r.d.tNew));
  let max=Math.max(...rows.map(r=>+(r.d.tClose||r.d.tRun||r.d.tNew)));
  max=min+(max-min)*1.03;
  const x=t=>(t-min)/(max-min)*100;
  const dayMs=86400000;
  const t0=new Date(min); t0.setHours(0,0,0,0);
  const ticks=[]; for(let t=+t0;t<=max;t+=7*dayMs) ticks.push(new Date(t));
  const ROW=30;
  const trunc=(s,n)=>s.length>n? s.slice(0,n)+'…':s;

  const gutter=rows.map(r=>`<div class="gcell ${sel===r.it.id?'on':''}" data-id="${r.it.id}">
      <span class="gid">${r.it.id.replace(/^0+/,'')}</span>${trunc(r.it.title,9)}</div>`).join('');
  const grids=ticks.map(tk=>`<div class="grid" style="left:${x(+tk)}%"></div>`).join('');
  const axis=ticks.map(tk=>`<div class="tick" style="left:${x(+tk)}%">${fmtDate(tk)}</div>`).join('');
  const bars=rows.map((r,i)=>{
    const s=+r.d.tNew, e=+(r.d.tClose||r.d.tRun||r.d.tNew);
    const left=x(s), w=Math.max(x(e)-x(s),0.6);
    const dur=r.it.running?'進行中':humanSpan(r.d.total)+' 跨度';
    return `<div class="gbar ${sel===r.it.id?'on':''}" data-id="${r.it.id}"
        style="left:${left}%;width:${w}%;top:${i*ROW+7}px" title="${r.it.id} ${r.it.title} · ${dur}">
        ${segmentBar(r.it,16)}</div>`;
  }).join('');

  document.getElementById('chart').innerHTML=`
    <div class="gantt">
      <div class="gutter"><div class="ghdr"></div>${gutter}</div>
      <div class="plot">
        <div class="axis">${axis}</div>
        <div class="bands" style="height:${rows.length*ROW}px">${grids}${bars}</div>
      </div>
    </div>`;

  document.querySelectorAll('.gbar,.gcell').forEach(el=>el.onclick=()=>{sel=el.dataset.id;render();});
  renderSel();
}

function renderSel(){
  const box=document.getElementById('sel');
  if(!sel){box.innerHTML=`<div class="selbox empty">點任一條 issue 看完整歷程</div>`;return;}
  const it=PROJECTS[cur].issues.find(x=>x.id===sel); const d=computeDurations(it);
  const spanTxt=it.running
    ? `已經過 <b>${humanSpan(d.elapsed)}</b>(未結算) · ${fmtDate(d.tNew)} 開始`
    : `總跨度 <b>${humanSpan(d.total)}</b> · ${fmtDate(d.tNew)} → ${fmtDate(d.tClose)} · token ${fmtTok(it.tokens)}`;
  box.innerHTML=`<div class="selbox">
    <div class="selhead"><div class="selid"><b>${it.id}</b> · ${it.slug}</div>
      <span class="badge ${it.running?'running':'closed'}">${it.running?'執行中 running':'已完成 closed'}</span></div>
    <div class="seltitle">${it.title}</div>
    <div class="selspan">${spanTxt}</div>
    ${detailHTML(it)}</div>`;
}
